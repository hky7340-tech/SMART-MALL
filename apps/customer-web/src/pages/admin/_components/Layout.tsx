...
const sidebarItems: SidebarSection[] = [
    ...
    },
    {
        ...
        items: [
            ...
            { href: '/admin/stores', label: 'Cửa hàng', icon: '🏪' },
            { href: '/admin/products', label: 'Sản phẩm', icon: '📦' },
            { href: '/admin/orders', label: 'Đơn hàng', icon: '📋', badge: '12', badgeColor: 'bg-red-500' },
    ...
    },
    {
        section: 'Khách hàng',
        items: [
            { href: '/admin/customers', label: 'CRM', icon: '🤝' },
    ...
    },
    {
        ...
        items: [
            ...
            { href: '/admin/gift-cards', label: 'Gift Card', icon: '🎁' },
            { href: '/admin/wallet', label: 'Ví điện tử', icon: '💰' },
    ...
    },
    {
        ...
        items: [
            ...
            { href: '/admin/notifications', label: 'Thông báo', icon: '🔔', badge: '5', badgeColor: 'bg-orange-500' },
        ]
    },
    {
        section: 'Dịch vụ',
        items: [
            { href: '/admin/restaurants', label: 'Nhà hàng', icon: '🍽️' },
            { href: '/admin/cinema', label: 'Rạp phim', icon: '🎬' },
            { href: '/admin/parking', label: 'Bãi xe', icon: '🅿️' },
            { href: '/admin/map', label: 'Bản đồ', icon: '🗺️' },
        ]
    },
    {
        section: 'Hỗ trợ',
        items: [
            { href: '/admin/chat', label: 'Quản lý Chat', icon: '💬', badge: '2', badgeColor: 'bg-yellow-500' },
        ]
    },
    {
        section: 'Báo cáo',
        items: [
            ...
            { href: '/admin/ai-insights', label: 'AI Insights', icon: '🤖' },
        ]
    },
    {
        section: 'Hệ thống',
        items: [
            ...
            { href: '/admin/roles', label: 'Phân quyền', icon: '🔐' },
        ]
    },
];

const quickActions = [
    { icon: '🎫', label: 'Tạo Voucher', href: '/admin/vouchers', color: 'bg-purple-100 text-purple-600' },
    { icon: '🔔', label: 'Gửi thông báo', href: '/admin/notifications', color: 'bg-blue-100 text-blue-600' },
    { icon: '📦', label: 'Thêm SP', href: '/admin/products', color: 'bg-green-100 text-green-600' },
    { icon: '🏪', label: 'Thêm CH', href: '/admin/stores', color: 'bg-orange-100 text-orange-600' },
];
        ...
        <div className="min-h-screen bg-gray-50 flex">
            ...
            `}>
                ...
                <div className={`h-16 flex items-center border-b border-gray-200 ${collapsed ? 'justify-center px-0' : 'px-5'}`}>
                    {collapsed ? (
                        <Link href="/admin" className="w-9 h-9 bg-primary-600 rounded-xl flex items-center justify-center flex-shrink-0">
                            ...
                            {section.items.map((item) => {
                                ...
                                return collapsed ? (
                                    <Link
                                        key={item.href}
                                        href={item.href}
                                        className={`flex items-center justify-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 mb-0.5 ${active ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
                                        title={item.label}
                                    >
                                    ...
                                    <Link
                                        key={item.href}
                                        href={item.href}
                                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 mb-0.5 ${active ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
                                    >
                                        <span className="text-lg">{item.icon}</span>
                            ...
                            {showUserMenu && (
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 z-50 overflow-hidden">
                                        ...
                                        <div className="py-1">
                                            {[
                                                { label: 'Hồ sơ', icon: '👤', href: '/admin/settings' },
                                                { label: 'Cài đặt tài khoản', icon: '⚙️', href: '/admin/settings' },
                                                { label: 'Lịch sử hoạt động', icon: '📋', href: '/admin/reports' },
                                            ].map(item => (
                                        ...
                                        <div className="border-t border-gray-100 py-1">
                                            <Link href="/" className="flex items-center gap-2.5 px-4 py-2 text-sm text-red-600 hover:bg-red-50 w-full transition-colors">
                                                <span>🚪</span>
                                                Về trang chủ
                                            </Link>
                                        </div>
