    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <button onClick={() => toast.success('Mở form thêm sản phẩm')} className="btn-primary">+ Thêm sản phẩm</button>
            </div>
                            ...
                            {filtered.map(p => (
                                <tr key={p.id}>
                                    ...
                                    <td className="text-right">
                                        <span className={`font-medium ${p.stock === 0 ? 'text-red-600' : p.stock < 50 ? 'text-yellow-600' : 'text-gray-900'}`}>{p.stock}</span>
                                    </td>
